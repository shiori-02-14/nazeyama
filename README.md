...
1. `content/site.yaml` を編集
2. 左の **ソース管理** を開く
3. **site.yaml にチェック**（`.DS_Store` は選ばない）
4. 上の **メッセージを入力** して **「コミット」**
5. **「変更の同期」**（雲のアイコン ↑）をクリック

### push できないとき

| 症状 | 対処 |
|---|---|
| 「同期」ボタンが出ない | まず **コミット** する |
| push が失敗する | **「プル」してから** もう一度「同期」 |
| `.DS_Store` だけ変更 | **無視してOK**（commit しない） |

`seo:` を変えたあと、LINE のプレビューをすぐ更新したい → **Actions → Update data → Run workflow**
